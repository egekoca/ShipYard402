...
const WITH: readonly { lead: string; highlight: string }[] = [
  {
    lead: 'Every release pays the real endpoint for real, then',
    highlight: 'tries to break it before a customer can.',
  },
  { lead: 'A broken payment check fails the release gate,', highlight: 'before it ever reaches someone paying.' },
  {
    lead: 'Every run leaves a signed, on-chain record:',
    highlight: 'what was tested, when, against which exact version.',
  },
];
...
export function ProblemSolution() {
  ...
  return (
    <section
      className={`problem-solution${visible ? ' is-visible' : ''}`}
      ref={ref}
      aria-label="The problem and how Shipyard402 solves it"
    >
      <div className="section-heading">
        ...
        <p>
          Most releases are tested for correctness. Almost none are tested for whether a paying customer can still
          actually pay.
        </p>
      </div>
