# Architecture Decision Records

- [ADR-0001](0001-product-scope.md): product scope and claim boundary
- [ADR-0002](0002-goat-flow-direct.md): public GOAT Flow payment mode
- [ADR-0003](0003-agentkit-example-boundary.md): AgentKit mock example boundary
- [ADR-0006](0006-ai-deterministic-boundary.md): AI versus deterministic authority
- [ADR-0009](0009-state-idempotency.md): state, outbox and idempotency
- [ADR-0012](0012-append-only-registry.md): append-only registry
- [ADR-0014](0014-runtime-payment-config.md): runtime payment configuration
- [ADR-0015](0015-frontend-backend-boundary.md): independently deploy frontend and backend
- [ADR-0016](0016-goat-flow-proof-verification.md): provider proof and on-chain settlement verification
- [ADR-0017](0017-goat-x402-api-origin.md): current official x402 API origins
