...
- GOAT Flow's order-proof `signature` is treated as an unsigned provider digest. Funding requires a chain-2345 receipt and an exact ERC-20 Transfer log match.
...
The payment worker exposes a queue-agnostic job handler with bounded retries and deterministic dead-letter rules. Its executable bootstrap intentionally remains disabled until a durable queue is wired; it is a separate backend deployment, not a Next.js background task.
...
Production boot remains disabled until the remaining quote/run PostgreSQL repositories, KMS/Vault signer references, and reviewed production merchant capability records are configured. The merchant adapter, order-context store, payment reconciler, and read-only chain receipt implementation are present but have not been exercised with production credentials.
