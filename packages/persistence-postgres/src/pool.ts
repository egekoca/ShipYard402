...
export async function assertShipyardSchemaReady(pool: Pool): Promise<void> {
  ...
  if (
    !state?.has_runs ||
    !state.has_payment_receipts ||
    !state.has_payment_jobs ||
    !state.has_orchestrator_jobs ||
    !state.has_orchestrator_checkpoints ||
    !state.has_migration_ledger
  ) {
    throw new Error('PostgreSQL schema is incomplete; run the checksum-verified migrations before startup');
  }
  const version = await pool.query(`SELECT 1 FROM shipyard_schema_migrations WHERE version = $1`, [
