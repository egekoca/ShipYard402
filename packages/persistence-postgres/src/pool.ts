...
export const SHIPYARD_REQUIRED_SCHEMA_VERSION = '0002_payment_reconciliation_jobs.sql';
...
export async function assertShipyardSchemaReady(pool: Pool): Promise<void> {
  ...
  if (!state?.has_runs || !state.has_payment_receipts || !state.has_payment_jobs || !state.has_migration_ledger) {
