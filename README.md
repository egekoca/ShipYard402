...
- `apps/orchestrator-worker`: leased-job worker that drives a `FUNDED` run through `ANALYZING → PLAN_COMPILED → PROCURING → EXECUTING → EVIDENCE_BUILDING → ATTESTING → DELIVERED_*` (see `docs/state-machine.md`). Calls a real OpenAI structured-outputs risk classifier whose proposal is advisory only — `@shipyard402/risk-classifier`'s deterministic compiler always keeps the mandatory scenario and clamps the proposed budget (ADR-0006). Pays `x402-demo-target` with a real signed GOAT Testnet3 transaction, runs the existing replay-check scenario against the earned receipt, signs an EIP-712 `ToolReceipt`, publishes the evidence pack to IPFS (a local Kubo node, see below), and submits a real EIP-712-signed attestation to the deployed `ShipyardRunRegistry` whose `evidenceURI` is that `ipfs://` CID — decentralized, content-addressed, and independently checkable by canonicalizing the fetched content and comparing its hash to the on-chain `contentHash`. Crash-resumable mid-sequence: each spend-once side effect (the procurement payment, the earned purchase receipt, the replay-probe result, the attestation submission) is checkpointed to PostgreSQL immediately after it happens, so a re-claimed job resumes from the run's persisted status instead of repeating them — a mid-pipeline failure now retries with backoff like any other job error rather than dead-lettering outright. The one disclosed residual gap is the narrow window between a payment or attestation transaction broadcasting successfully on-chain and the worker persisting its hash locally; a crash in that exact instant would still need manual reconciliation against chain state.
