...
// Mirrors scripts/testnet/deploy-test-token.mjs's GOAT Testnet3 test token, pointed at BOT Chain
// testnet instead -- BOT Chain's own testnet has no known stablecoin/ERC20 address published (its
// dev docs only list mainnet WBOT/USDT contracts), so local end-to-end testing needs our own
// throwaway mintable token, exactly the way the GOAT setup already does.
const RPC_URL = 'https://rpc.bohr.life';
const EXPECTED_CHAIN_ID = 968n;
// BOT Chain gas is priced far higher (in native BOT) than GOAT's BTC-denominated gas; the GOAT
// script's tiny cap does not transfer. 0.2 BOT gives headroom while still failing closed.
...
if (receipt?.status !== 1 || transaction.chainId !== EXPECTED_CHAIN_ID) {
  throw new Error('Token deployment was not successful on BOT Chain testnet');
}
...
async function loadExistingDeployment() {
  try {
    ...
    if (value.environment !== 'botchain-testnet' || value.chainId !== Number(EXPECTED_CHAIN_ID)) {
      throw new Error('Existing test token deployment is not scoped to BOT Chain testnet');
    }
    return value;
  } catch (error) {
